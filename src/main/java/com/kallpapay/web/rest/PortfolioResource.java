...
/**
 * REST controller for managing {@link com.kallpapay.domain.Portfolio}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Portfolio> partialUpdatePortfolio(
        ...
        Optional<Portfolio> result = portfolioRepository
            ...
            .map(existingPortfolio -> {
                if (portfolio.getLimitAmount() != null) {
                    existingPortfolio.setLimitAmount(portfolio.getLimitAmount());
                }
                if (portfolio.getAmount() != null) {
                    existingPortfolio.setAmount(portfolio.getAmount());
                }
                if (portfolio.getLimitPercentage() != null) {
                    existingPortfolio.setLimitPercentage(portfolio.getLimitPercentage());
                }
                if (portfolio.getPercentage() != null) {
                    existingPortfolio.setPercentage(portfolio.getPercentage());
                }
                if (portfolio.getMethod() != null) {
                    existingPortfolio.setMethod(portfolio.getMethod());
                }
